...
isc_counter_create(isc_mem_t *mctx, int limit, isc_counter_t **counterp) {
	...
	isc_refcount_init(&counter->used, 0);
...
isc_counter_increment(isc_counter_t *counter) {
	uint32_t cur = isc_refcount_increment(&counter->used) + 1;

	return ((counter->limit != 0 && (cur >= atomic_load(&counter->limit)))
		? ISC_R_QUOTA
		: ISC_R_SUCCESS);
}
...
isc_counter_used(isc_counter_t *counter) {
	...
	return (isc_refcount_current(&counter->used));
}
