...
#define isc_refcount_current(target)				\
	(uint_fast32_t)atomic_load_explicit(target, memory_order_acq_rel)
...
#define isc_refcount_increment0(target)				\
	atomic_fetch_add_explicit(target, 1, memory_order_acq_rel)
...
#define isc_refcount_decrement(target)				\
	atomic_fetch_sub_explicit(target, 1, memory_order_acq_rel)
